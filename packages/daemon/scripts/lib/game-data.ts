import { getUnitStat, getDifficulty, getUnitHP, getUnitMaxHP, getUnitMP } from "diablo:native";
...
function skillLevel(skillId: number): number {
  return getUnitStat(skillId, 0); // TODO: use proper getSkill(skillId, 1) when available
}

function baseLevel(skillId: number): number {
  return getUnitStat(skillId, 0); // base points (no +skills)
}
