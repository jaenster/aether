...
static bool intrinsic_IsWrappedArrayConstructor(JSContext* cx, unsigned argc,
                                                Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  bool result = false;
  if (!IsWrappedArrayConstructor(cx, args[0], &result)) return false;
...
static bool intrinsic_ToInteger(JSContext* cx, unsigned argc, Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  double result;
  if (!ToInteger(cx, args[0], &result)) return false;
...
static bool intrinsic_ToPropertyKey(JSContext* cx, unsigned argc, Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  RootedId id(cx);
  if (!ToPropertyKey(cx, args[0], &id)) return false;
...
static bool intrinsic_OwnPropertyKeys(JSContext* cx, unsigned argc, Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  MOZ_ASSERT(args.length() == 1);
  MOZ_ASSERT(args[0].isObject());
  RootedObject obj(cx, &args[0].toObject());
  return GetOwnPropertyKeys(
      cx, obj, JSITER_OWNONLY | JSITER_HIDDEN | JSITER_SYMBOLS, args.rval());
}
...
static bool intrinsic_DefineDataProperty(JSContext* cx, unsigned argc,
  ...
  RootedObject obj(cx, &args[0].toObject());
  RootedId id(cx);
  if (!ValueToId<CanGC>(cx, args[1], &id)) return false;
  ...
  MOZ_ASSERT(bool(attributes & ATTR_ENUMERABLE) !=
             ...
             "ATTR_NONENUMERABLE");
  if (attributes & ATTR_ENUMERABLE) attrs |= JSPROP_ENUMERATE;
...
static bool intrinsic_DefineProperty(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (attributes & ACCESSOR_DESCRIPTOR_KIND) {
    ...
    if (getter.isObject()) desc.setGetterObject(&getter.toObject());
    if (!getter.isNull()) attrs |= JSPROP_GETTER;

    Value setter = args[4];
    MOZ_ASSERT(setter.isObject() || setter.isNullOrUndefined());
    if (setter.isObject()) desc.setSetterObject(&setter.toObject());
    if (!setter.isNull()) attrs |= JSPROP_SETTER;
  ...
  if (!DefineProperty(cx, obj, id, desc, result)) return false;

  bool strict = args[5].toBoolean();
  if (strict && !result.checkStrict(cx, obj, id)) return false;
...
static bool intrinsic_ObjectHasPrototype(JSContext* cx, unsigned argc,
  ...
  if (!GetPrototype(cx, obj, &actualProto)) return false;
...
static bool intrinsic_UnsafeGetObjectFromReservedSlot(JSContext* cx,
                                                      unsigned argc,
                                                      Value* vp) {
  if (!intrinsic_UnsafeGetReservedSlot(cx, argc, vp)) return false;
...
template <typename T>
static bool intrinsic_IsWrappedArrayBuffer(JSContext* cx, unsigned argc,
                                           Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  MOZ_ASSERT(args.length() == 1);
...
static bool intrinsic_IsSpecificTypedArray(JSContext* cx, unsigned argc,
                                           Value* vp, Scalar::Type type) {
  CallArgs args = CallArgsFromVp(argc, vp);
  MOZ_ASSERT(args.length() == 1);
  MOZ_ASSERT(args[0].isObject());
...
static bool intrinsic_IsUint8TypedArray(JSContext* cx, unsigned argc,
                                        Value* vp) {
...
static bool intrinsic_IsInt8TypedArray(JSContext* cx, unsigned argc,
                                       Value* vp) {
  return intrinsic_IsSpecificTypedArray(cx, argc, vp, Scalar::Int8);
}

static bool intrinsic_IsUint16TypedArray(JSContext* cx, unsigned argc,
                                         Value* vp) {
  return intrinsic_IsSpecificTypedArray(cx, argc, vp, Scalar::Uint16);
}

static bool intrinsic_IsInt16TypedArray(JSContext* cx, unsigned argc,
                                        Value* vp) {
  return intrinsic_IsSpecificTypedArray(cx, argc, vp, Scalar::Int16);
}

static bool intrinsic_IsUint32TypedArray(JSContext* cx, unsigned argc,
                                         Value* vp) {
  return intrinsic_IsSpecificTypedArray(cx, argc, vp, Scalar::Uint32);
}

static bool intrinsic_IsInt32TypedArray(JSContext* cx, unsigned argc,
                                        Value* vp) {
  return intrinsic_IsSpecificTypedArray(cx, argc, vp, Scalar::Int32);
}

static bool intrinsic_IsFloat32TypedArray(JSContext* cx, unsigned argc,
                                          Value* vp) {
  return intrinsic_IsSpecificTypedArray(cx, argc, vp, Scalar::Float32);
}

static bool intrinsic_IsPossiblyWrappedTypedArray(JSContext* cx, unsigned argc,
                                                  Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  MOZ_ASSERT(args.length() == 1);
...
static bool intrinsic_SetFromTypedArrayApproach(JSContext* cx, unsigned argc,
  ...
  if (!unsafeTypedArrayCrossCompartment) return false;
  ...
  SharedMem<uint8_t*> targetData = target->viewDataEither().cast<uint8_t*>() +
  ...
  SharedMem<uint8_t*> targetDataLimit =
      target->viewDataEither().cast<uint8_t*>() +
...
static void CopyToDisjointArray(TypedArrayObject* target, uint32_t targetOffset,
  ...
  SharedMem<uint8_t*> dest = target->viewDataEither().cast<uint8_t*>() +
...
void js::SetDisjointTypedElements(TypedArrayObject* target,
  ...
  SharedMem<void*> unsafeSrcDataCrossCompartment =
      unsafeSrcCrossCompartment->viewDataEither();
...
static bool intrinsic_SetDisjointTypedElements(JSContext* cx, unsigned argc,
  ...
  if (!unsafeSrcCrossCompartment) return false;
...
bool CallSelfHostedNonGenericMethod(JSContext* cx, const CallArgs& args) {
  ...
  InvokeArgs args2(cx);
  if (!args2.init(cx, args.length() - 1)) return false;

  for (size_t i = 0; i < args.length() - 1; i++) args2[i].set(args[i]);
...
static bool intrinsic_WarnDeprecatedStringMethod(JSContext* cx, unsigned argc,
  ...
  if (!(cx->compartment()->warnedAboutStringGenericsMethods & mask)) {
    ...
    if (!stableChars.initTwoByte(cx, name)) return false;
...
static bool intrinsic_ConstructFunction(JSContext* cx, unsigned argc,
  ...
  if (!constructArgs.init(cx, len)) return false;
  for (uint32_t index = 0; index < len; index++)
    constructArgs[index].set(argsList->getDenseElement(index));

  RootedObject res(cx);
  if (!Construct(cx, args[0], constructArgs, args[1], &res)) return false;
...
static const JSFunctionSpec intrinsic_functions[] = {
    ...
    JS_INLINABLE_FN("std_SIMD_Int32x4_extractLane", simd_int32x4_extractLane, 2,
                    0, SimdInt32x4_extractLane),
    ...
    JS_INLINABLE_FN("std_SIMD_Float32x4_extractLane",
                    simd_float32x4_extractLane, 2, 0,
    ...
    JS_INLINABLE_FN("IsWrappedArrayConstructor",
                    intrinsic_IsWrappedArrayConstructor, 1, 0,
    ...
    JS_FN("IsArrayBuffer", intrinsic_IsInstanceOfBuiltin<ArrayBufferObject>, 1,
          0),
    ...
    JS_INLINABLE_FN("IsPossiblyWrappedTypedArray",
                    intrinsic_IsPossiblyWrappedTypedArray, 1, 0,
...
#undef LOAD_AND_STORE_REFERENCE_FN_DECLS
    ...
    JS_FN("IsPromiseObject", intrinsic_IsInstanceOfBuiltin<PromiseObject>, 1,
          0),
...
static void MaybePrintAndClearPendingException(JSContext* cx, FILE* file) {
  if (!cx->isExceptionPending()) return;
...
bool JSRuntime::isSelfHostingCompartment(JSCompartment* comp) const {
...
static bool CloneProperties(JSContext* cx, HandleNativeObject selfHostedObject,
  ...
  for (size_t i = 0; i < selfHostedObject->getDenseInitializedLength(); i++) {
    if (!selfHostedObject->getDenseElement(i).isMagic(JS_ELEMENTS_HOLE)) {
      if (!ids.append(INT_TO_JSID(i))) return false;
      if (!attrs.append(JSPROP_ENUMERATE)) return false;
    }
  }
  ...
  for (Shape::Range<NoGC> range(selfHostedObject->lastProperty());
    ...
    if (shape.enumerable() && !shapes.append(&shape)) return false;
  }
  ...
  for (size_t i = 0; i < shapes.length(); ++i) {
    ...
    if (!ids.append(shapes[i]->propid())) return false;
    uint8_t shapeAttrs =
        shapes[i]->attributes() &
        (JSPROP_ENUMERATE | JSPROP_PERMANENT | JSPROP_READONLY);
    if (!attrs.append(shapeAttrs)) return false;
  }
...
static JSString* CloneString(JSContext* cx, JSFlatString* selfHostedString) {
  ...
  if (!chars.init(cx, selfHostedString)) return nullptr;
...
static JSObject* CloneObject(JSContext* cx,
  ...
  if (selfHostedObject->is<JSFunction>()) {
    ...
    clone = RegExpObject::create(cx, source, reobj.getFlags(),
                                 cx->tempLifoAlloc(), TenuredObject);
    ...
    if (!selfHostedString->isFlat()) MOZ_CRASH();
  ...
  if (!CloneProperties(cx, selfHostedObject, clone)) return nullptr;
  return clone;
}
...
static bool CloneValue(JSContext* cx, HandleValue selfHostedValue,
  ...
  if (selfHostedValue.isObject()) {
    ...
    if (!selfHostedValue.toString()->isFlat()) MOZ_CRASH();
...
JSFunction* JSRuntime::getUnclonedSelfHostedFunction(JSContext* cx,
                                                     HandlePropertyName name) {
  RootedValue selfHostedValue(cx);
  if (!getUnclonedSelfHostedValue(cx, name, &selfHostedValue)) return nullptr;
...
bool JSRuntime::cloneSelfHostedValue(JSContext* cx, HandlePropertyName name,
                                     MutableHandleValue vp) {
  RootedValue selfHostedValue(cx);
  if (!getUnclonedSelfHostedValue(cx, name, &selfHostedValue)) return false;
...
JSFunction* js::SelfHostedFunction(JSContext* cx, HandlePropertyName propName) {
  RootedValue func(cx);
